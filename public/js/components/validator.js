...
module.exports = (options) => {
	...
	var validate = (self) => {
		...
		if (element === 'input' || element === 'select' || element === 'textarea') {
			var errorElement = jQuery('[data-id=' + self.attr('id') + ']');

			if (errorElement.length === 0) {
				self.after('<div data-id="' + self.attr('id') + '" class="' + settings.errorclass.join(' ') + '"></div>');
				errorElement = jQuery('[data-id=' + self.attr('id') + ']');
			}
				...
				for (var i = 0; i < ruleArray.length; i++) {
					...
					switch (rule) {
						...
						case 'pattern':
							...
							break;
						case 'different':
							if (value === jQuery('[name=' + parameters + ']').val()) {
	...
	if (settings.blur) {
		jQuery('input, select, textarea').blur((event) => {
			validate(jQuery(event.currentTarget));
		});
	}
	if (settings.keypress) {
		jQuery('input, select, textarea').keyup((event) => {
	...
	if (settings.submit) {
		jQuery('form').submit((event) => {
			jQuery(event.currentTarget).find('input, textarea, select').each((index, event) => {
				validate(jQuery(event));
			});
			if (jQuery(event.currentTarget).find('[data-id]:visible').length > 0) {
				event.preventDefault();
				jQuery(event.currentTarget).find('[data-id]:visible').first().prev().focus();
				jQuery(event.currentTarget).trigger('invalid');
			} else {
				jQuery(event.currentTarget).trigger('valid');
				return true;
			}
		});
	}
