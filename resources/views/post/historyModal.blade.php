		...
		<div class="wrapper">
			...
			<ul>
				@foreach($post->revisionHistory->sortByDesc('created_at')->take(100) as $history )
					<li class="margin-bottom-half">{{$history->created_at->format('Y-m-d')}} - {{HTML::actionlink($url = array('action' => 'UserController@show', 'params' => array($history->userResponsible()->id)), $history->userResponsible()->username)}} changed <strong>{{ $history->fieldName() }}</strong> from <code>{{ $post->getAnswer($history->oldValue()) }}</code> to <code>{{ $post->getAnswer($history->newValue()) }}</code>.
